...
from cart_change import cart, Item, Shop, items
...
class CartTest(unittest.TestCase):
    ...
    def test_add1(self):
        test_cart.add('blueberry', 4, 3, items)
        self.assertIn('blueberry', items)
    
    def test_add2(self):
        test_cart.add('blueberry', 4, 3, items)
        self.assertNotIn('soles', items)
        self.assertIn('blueberry', items)
    
    def test_int(self):
        johnson = Item('johnson', 'coolcat', 'jimmy')
        self.assertIsInstance(johnson.quantity, int)

    def test_delete1(self):
        test_cart.add('mango', 4, 3, items)
        self.assertIn('mango', items)
        test_cart.delete('mango', items)
        self.assertNotIn('mango', items)
